...
- `useChatStream()` 单会话流式聊天 composable
...
- 单会话 `useChatStream()` 状态管理

## 分层模型

`one-chat` 按从内到外的方式组织能力，每一层都可以独立使用：

- `useChatStream()`：最小聊天闭环，只管理当前消息、流式状态、错误和重试。
...
- 不需要助手列表时，直接使用 `useChatConversation() + useChatStream()`
- 不需要会话列表时，直接使用 `useChatStream()`

## 设计约束

- `useChatStream()` 不依赖助手或会话列表，只接收可选的 `assistantId` 与 `conversationId`
